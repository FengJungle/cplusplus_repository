...
namespace mySTL
{
    ...
    private:
        ...
        void reallocate_map(size_t nodes_to_add, bool add_at_front)
        {
            ...
            // 当前map的头部空间不够，但其实map的空间足够大，只是大部分数据填充在map的头部
            // 调整start-finish至map的中央区间，使调整后的map头尾剩余空间相当
            ...
            // 当前map头部无空间，且整个map剩余空间也不多，需要重新配置更大的map，并将原map的内容拷贝到新map
            else
            {
    ...
    public:
        ...
        my_deque():start(), finish(), map(0), map_size(0)
        {
            fill_initialize(0, T());
        }
