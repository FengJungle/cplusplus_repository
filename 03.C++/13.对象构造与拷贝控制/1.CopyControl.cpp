...
注意：拷贝构造函数被用来初始化 非引用 类类型参数，这一特性解释了“拷贝构造函数自己的参数必须是引用类型”，
     ...
     —— 为了调用拷贝构造函数，我们必须拷贝它的实参，但为了拷贝实参，我们又要调用拷贝构造函数，如此循环往复。

容器的insert和push操作是将元素类型的对象传递给它们，这些对象被拷贝到容器中。（拷贝初始化）
容器的emplace操作是将参数传递给元素类型的构造函数，emplace成员使用这些参数在容器管理的内存空间中直接构造元素（直接初始化）
