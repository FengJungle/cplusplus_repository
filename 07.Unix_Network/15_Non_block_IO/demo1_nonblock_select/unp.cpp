...
int udp_client(const char *host, const char *serv, void **saptr, socklen_t *lenp)
{
	...
	if ((n = getaddrinfo(host, serv, &hints, &res)) != 0)
	{
		cout << "udp_client error: " << gai_strerror(n) << endl;
		return -1;
	}
...
int udp_connect(const char *host, const char *serv)
{
	...
	if ((n = getaddrinfo(host, serv, &hints, &res)) != 0)
	{
		cout << "udp_conect failed with error:" << gai_strerror(n) << endl;
		return -1;
	}
	ressave = res;
...
int udp_server(const char *host, const char *serv, socklen_t *addrlenp)
{
	...
	if ((n = getaddrinfo(host, serv, &hints, &res)) != 0)
	{
		cout << "udp_server failed with error:" << gai_strerror(n) << endl;
		return -1;
	}
