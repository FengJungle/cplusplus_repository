...
int main(int argc, char *argv[])
{
    ...
    addr_serv.sin_addr.s_addr = inet_addr(SERVER_IP_ADDR);
    ...
    struct epoll_event events[MAX_EVENT]; // 128
    ...
    if(epoll_ctl(nEpollfd, EPOLL_CTL_ADD, STDIN_FILENO, &ev) < 0)
    {
        printf("[%s %d] Epoll ctl error!\n", __FUNCTION__, __LINE__);
        return 0;
    }
    ...
    int nSendNum;
    int nRecvNum;
    char szSendBuf[MAX_BUFSIZE];//= "NonBlocking Epoll Test!";
